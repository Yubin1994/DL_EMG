...
path='G:\\吴乐\\包络18_03_07\\baseline_50ms_10x10_Norm\\data_train'
data=sio.loadmat(path)
X_train = data['data_train']
X_train = X_train[...,np.newaxis]

path='G:\\吴乐\\包络18_03_07\\baseline_50ms_10x10_Norm\\label_train'
data=sio.loadmat(path)
label_train = ((data['label_train']).T )
#path='G:\\吴乐\\10_10_ULDA_LDC\\train_d'
#data=sio.loadmat(path)
#X_train = data['train_d']
#
#path='G:\\吴乐\\10_10_ULDA_LDC\\train_label'
#data=sio.loadmat(path)
#label_train = ((data['train_label']-1).T )
...
#    X = Activation('relu')(X)
    ...
    X = Conv2D(256,(3,3),strides = (1, 1),name='fc1',kernel_regularizer=regularizers.l2(0.05))(X)
    X = Conv2D(64,(1,1),strides = (1, 1),name='fc2',kernel_regularizer=regularizers.l2(0.05))(X)
    X = Conv2D(7,(1,1),strides = (1, 1),name='fc3',kernel_regularizer=regularizers.l2(0.05))(X) # 1*1*11
...
SemgModel.fit(x=X_train,y=Y_train,epochs=15,batch_size=64,shuffle=True,validation_split=0.1)


adam = optimizers.Adam(lr=0.00001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
SemgModel.compile(optimizer =adam, loss ="categorical_crossentropy",metrics =["accuracy"])
SemgModel.fit(x=X_train,y=Y_train,epochs=40,batch_size=64,shuffle=True,validation_split=0.1)

## 用于保存参数
SemgModel.save_weights('emgModel_7cls_10x10_50ms.h5')
